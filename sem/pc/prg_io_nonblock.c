...
{
    // as defined for FIFO, open will be blocked for read only unless nonblock
    // is specified
...
{
    // Be aware that the opening named pipe for writing is blocked until the
    // pipe is opened for reading. Thus, run a program that opens the pipe or
    // call, e.g., 'tail -f fname', where 'fname' is the filename name of the
    // named pipe being opened for writing.
